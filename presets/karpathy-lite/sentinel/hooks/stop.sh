#!/usr/bin/env bash
# presets/karpathy-lite/sentinel/hooks/stop.sh
# Sentinel Stop hook. Reads JSON from stdin, emits an event
...
observers="$(sentinel_observers_subscribed_to stop)"
[ -z "$observers" ] && exit 0

for obs in $observers; do
  if sentinel_should_dispatch "$session_id" "$obs" stop "$tool_name" "$stdin"; then
    eid="$(printf '%s%s' "$(date +%s%N)" "$$")"
    sentinel_emit_event "$session_id" "$eid" "$obs" stop "$tool_name" "$stdin" "$transcript"
  fi
done

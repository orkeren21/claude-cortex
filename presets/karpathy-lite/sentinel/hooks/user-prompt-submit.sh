#!/usr/bin/env bash
# presets/karpathy-lite/sentinel/hooks/user-prompt-submit.sh
# Sentinel UserPromptSubmit hook. Reads JSON from stdin, emits an event
...
observers="$(sentinel_observers_subscribed_to user_prompt_submit)"
[ -z "$observers" ] && exit 0

for obs in $observers; do
  if sentinel_should_dispatch "$session_id" "$obs" user_prompt_submit "$tool_name" "$stdin"; then
    eid="$(printf '%s%s' "$(date +%s%N)" "$$")"
    sentinel_emit_event "$session_id" "$eid" "$obs" user_prompt_submit "$tool_name" "$stdin" "$transcript"
  fi
done
